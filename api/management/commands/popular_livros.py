...
from api.models import Editora, Autor, Livros
...
class Command(BaseCommand):
    ...
    def handle(self, *a, **o):
        ...
        # 4) Buscar auyor de livros no dicionario e retornar "id"
        ...
        # 3) Criar o "id_editoras" em livros
        df_livros['id_editora'] = df_livros["editora"].map(mapa_editoras)

        if o["truncate"]:Livros.objects.all().delete
        ...
        if o["update"]: 
            ...
            for r in df_livros.itertuples(index=False):
                _, created = Livros.objects.update_or_create(
            ...
            Livros.objects.bulk_create(objs, ignore_conflicts=True)
            criados = len(objs)
        ...
        # else:
        #     objs = [Editora(
        #             editora = r.editoras, cnpj = r.cnpj, endereco = r.endereco, 
